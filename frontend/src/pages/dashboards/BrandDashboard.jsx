...
import { FaBoxOpen, FaChartLine, FaShoppingBag, FaDollarSign, FaTags, FaArrowRight, FaRocket, FaClock, FaExclamationTriangle, FaCheckCircle } from "react-icons/fa";
...
const BrandDashboard = () => {
    ...
    const quickActions = [
        { to: "/brand/inventory", icon: <FaBoxOpen />, title: "Manage Inventory", desc: "Update stock & add products", color: "teal" },
        { to: "/brand/sales", icon: <FaChartLine />, title: "View Sales Report", desc: "Analyze performance details", color: "violet" },
    ];

    return (
        <div className="space-y-8 max-w-7xl mx-auto">
            {/* Welcome Section - Date Context */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-900">
                        Hello, <span className="text-teal-600">{user?.name || "Partner"}</span> 👋
                    </h1>
                    <p className="text-slate-500 text-sm mt-1">Here is what's happening with your store today.</p>
                </div>
                ...
                {statCards.map((stat, i) => (
                    <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all group">
                        ...
                        <div className="mt-4 flex items-center text-xs text-slate-400 font-medium">
                            <span className="text-teal-500 flex items-center gap-1 bg-teal-50 px-1.5 py-0.5 rounded">
                                <FaChartLine /> +2.5%
                            </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                    <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        ...
                        <Link to="/brand/sales" className="text-xs font-semibold text-teal-600 hover:text-teal-700 hover:underline">
                            View All Orders
                        </Link>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                        ...
                        <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 border border-amber-100">
                            ...
                            {lowStockCount > 0 && (
                                <Link to="/brand/inventory" className="mt-4 block w-full py-2 bg-white text-amber-600 text-center rounded-lg text-sm font-bold shadow-sm hover:shadow hover:bg-amber-50 transition-all">
                                    Restock Now
                                </Link>
                            )}
                        </div>
